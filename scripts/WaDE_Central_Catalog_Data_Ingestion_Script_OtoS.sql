...
DO $$ 
    BEGIN
        BEGIN
		INSERT INTO "WADE"."XMLContent" ("XML_CONTENT")
		SELECT content FROM http_get('http://www.westernstateswater.org/SouthDakota/WADE/v0.2/GetCatalog/GetCatalog_GetAll.php?orgid=DENR');
	EXCEPTION
		WHEN internal_error THEN RAISE NOTICE 'Problem while connecting to Oregon Web service';
        END;
    END;
...
--DROP TABLE IF EXISTS "WADE"."CATALOG_SUMMARY_MV_OTOS";

CREATE TABLE "WADE"."CATALOG_SUMMARY_MV_OTOS"
(
  "ID" serial NOT NULL,
  ...
  CONSTRAINT "CATALOG_SUMMARY_MV_OTOS_pkey" PRIMARY KEY ("ID")
)
WITH (
  OIDS=FALSE
);
ALTER TABLE "WADE"."CATALOG_SUMMARY_MV_OTOS"
  OWNER TO postgres;
GRANT ALL ON TABLE "WADE"."CATALOG_SUMMARY_MV_OTOS" TO postgres;
GRANT ALL ON TABLE "WADE"."CATALOG_SUMMARY_MV_OTOS" TO admin;
GRANT SELECT ON TABLE "WADE"."CATALOG_SUMMARY_MV_OTOS" TO "WADE_APP";
...
INSERT INTO "WADE"."CATALOG_SUMMARY_MV_OTOS" ("ORGANIZATION_ID", "WADE_URL", "REPORT_ID", "DATACATEGORY", "DATATYPE", "STATE", "REPORT_UNIT_ID", "REPORTING_UNIT_NAME", "COUNTY_FIPS", "HUC")
...
DELETE FROM "WADE"."CATALOG_SUMMARY_MV_OTOS"
