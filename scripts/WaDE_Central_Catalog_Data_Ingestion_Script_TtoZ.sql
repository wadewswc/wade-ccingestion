...
DO $$ 
    BEGIN
        BEGIN
		INSERT INTO "WADE"."XMLContent" ("XML_CONTENT")
		SELECT content FROM http_get('https://fortress.wa.gov/ecy/wade/v0.2/GetCatalog/GetCatalog_GetAll?orgid=WAECY');
	EXCEPTION
		WHEN internal_error THEN RAISE NOTICE 'Problem while connecting to Wyoming WYSEO Web service';
        END;
    END;
...
(
  ...
  CONSTRAINT "CATALOG_SUMMARY_MV_TTOZ_pkey" PRIMARY KEY ("ID")
)
WITH (
  OIDS=FALSE
);
ALTER TABLE "WADE"."CATALOG_SUMMARY_MV_TTOZ"
  OWNER TO postgres;
GRANT ALL ON TABLE "WADE"."CATALOG_SUMMARY_MV_TTOZ" TO postgres;
GRANT ALL ON TABLE "WADE"."CATALOG_SUMMARY_MV_TTOZ" TO admin;
GRANT SELECT ON TABLE "WADE"."CATALOG_SUMMARY_MV_TTOZ" TO "WADE_APP";
...
INSERT INTO "WADE"."CATALOG_SUMMARY_MV_TTOZ" ("ORGANIZATION_ID", "WADE_URL", "REPORT_ID", "DATACATEGORY", "DATATYPE", "STATE", "REPORT_UNIT_ID", "REPORTING_UNIT_NAME", "COUNTY_FIPS", "HUC")
...
DELETE FROM "WADE"."CATALOG_SUMMARY_MV_TTOZ"
